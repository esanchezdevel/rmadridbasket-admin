...
public class ProcessEuroleagueGame implements ProcessJsoup {
	...
	public void process(String url) {
		try {
			...
			fillPlayerStatsListService.setParameters(FillPlayerStatsListService.TYPE_EUROLEAGUE, 2, tables, true, playerStatsList);
			fillPlayerStatsListService.execute();
			
			//TODO check that the playerStatsList is updated without get it.
			
			fillPlayerStatsListService.setParameters(FillPlayerStatsListService.TYPE_EUROLEAGUE, 3, tables, false, playerStatsList);
			fillPlayerStatsListService.execute();
